...
- **종료 상태(end-state, Phase 5f 실현)**: Bun 런타임 parity 검증 후, `ccr.sh`를 대체하는 대신 **유지**하기로 결정했다(설치 스크립트 가치 보존). `ccr.sh`는 여전히 진실 공급원(bin 래퍼·슬래시 명령·config 히어독)이며, 런타임은 Python `ccr-hook.py`에서 Bun 번들 `ccr-cli.js`로 교체되었다. 번들은 `ccr.sh`에 인라인 임베드하지 않고 `templates/bin/ccr-cli.js`로 분리 산출하여 `ccr.sh`/`install.ts`가 복사 설치한다(중복·거대 디프 제거; 계획의 allowlist 의도). 드리프트 가드: `bundle-sync.test.ts`(`src/cli.ts` 신선 빌드 == 커밋된 번들)와 `check:sync`(나머지 43개 `templates/` == `ccr.sh` 히어독). 번들 갱신은 `bun run build:cli`. Python은 설치·런타임·테스트에서 완전히 제거되었다.
...
### Phase 5f 산출물 (Python 런타임 제거 / 컷오버)

마이그레이션의 종결 단계. Bun 런타임을 진실 공급원으로 승격하고 Python을 설치·런타임 양쪽에서 완전히 제거했다. `ccr.sh`는 (계획대로) **유지**하되 페이로드를 교체했다.

- **단일 번들(분리 산출, 비임베드)** — `scripts/build-cli.ts`(Bun.build, 무압축·결정적)가 `src/cli.ts`를 의존성 인라인 단일 `templates/bin/ccr-cli.js`(약 162KB)로 빌드한다(기본 출력 경로 = 해당 파일). **계획의 "[check:sync 허용]" 의도대로 번들은 `ccr.sh`에 인라인 임베드하지 않는다** — `ccr.sh`는 기존 `ccr-hook.py` 히어독을, 자기 체크아웃(`$SCRIPT_DIR/templates/bin/ccr-cli.js`)에서 `cp`로 복사 설치하는 블록으로 교체했다(번들 미존재 시 명확한 에러로 종료). 이로써 (1) `ccr.sh` 디프가 작아 리뷰 가능, (2) 번들 중복(ccr.sh+templates 이중 보관) 제거. `check-templates-sync.ts`는 `ccr-cli.js`를 allowlist 처리(heredoc emit 수 44→43). 번들은 `.gitattributes`에서 `-diff linguist-generated`로 표시해 생성물로 취급한다.
- **bin 재결선** — 22개 래퍼(`ccr-hook-claude`/`ccr-hook-codex` + 20개 `ccr-*` 명령)가 `exec python3 …/ccr-hook.py` → `exec bun …/ccr-cli.js`로 전환. `need_cmd python3` → `need_cmd bun`.
- **3개 install-time Python 블록 → bun** — (1) settings-merge(`python3 <<'PY'`)는 인라인 `bun run - <<'JS'`(install.ts `mergeSettings`와 동일 로직·키순서·한국어 메시지)로, (2) install-selftest(`python3 - <<'SELFTEST'`)는 `bun "$BIN_ROOT/ccr-cli.js" --command selftest`로(설치된 번들 자체 셀프테스트 재실행), (3) json-validate(`python3 - … <<'PY'`)는 인라인 `bun run - … <<'JS'`로 전환. `ccr.sh`는 이제 Python을 전혀 호출하지 않는다(`bash -n` 통과).
- **install.ts de-python** — `needCmd("python3")` → `needCmd("bun")`; step 7 selftest를 `bun ccr-cli.js --command selftest` 스폰으로 교체. `constants.ts` `GENERATED_BIN_NAMES`의 `ccr-hook.py` → `ccr-cli.js`. `uninstall.ts`는 구(pre-5f) `ccr-hook.py`를 레거시 정리 목록으로 제거(현행 doctor 누락검사에는 미포함).
- **제거** — `templates/bin/ccr-hook.py`(154KB), `templates/selftest-install.py`. `check-templates-sync.ts`의 SELFTEST 특례·allowlist 항목 제거(emit 파일 수는 ccr-hook.py↔ccr-cli.js 상쇄로 44 유지).
- **드리프트 가드 도구화** — `scripts/build-cli.ts`(기본 출력 = `templates/bin/ccr-cli.js`)로 번들 재생성, `scripts/extract-templates.ts`(ccr.sh→templates/ 재추출, check:sync의 역연산). 신뢰 사슬: `src/cli.ts` —(`bundle-sync.test.ts`: 번들 == 신선 빌드)→ `templates/bin/ccr-cli.js` —(install.ts/ccr.sh가 그대로 복사)→ 설치본. 나머지 43개 템플릿은 `check:sync`로 `ccr.sh` 히어독과 바이트 일치 유지.
- **테스트 전환(오라클 → TS-only)** — Python 오라클이 사라지므로 모든 Python-parity 테스트(golden/differential/phase3/phase4*/phase5a–d + golden-probe.py/phase3-probe.py/phase3-lock-runner.ts/integration_loop.py/loop.test.ts/golden 픽스처)를 삭제. 남은 회귀는 TS-only: `phase5e`(번들≡소스), `request-contract`, `install`, `constants`(pyInt + 고정 기본값), 신규 `bundle-sync`(번들 드리프트), 신규 `install-e2e`(격리 HOME에서 실제 `bash ccr.sh` 전 과정 + 런타임 실행). 풀 스위트 26 pass/0 fail; `typecheck`/`check:sync` green.
  - 비고(수용된 커버리지 트레이드오프): Python 오라클 차등·통합 커버리지(특히 `integration_loop.py`의 worker↔reviewer 핸드오프 회귀)는 오라클 제거와 함께 소실. 향후 필요 시 `phase5e` 류 TS-self 시나리오로 재구축 가능.
